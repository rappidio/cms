...
var Content = Base.inherit("metaDataParser.Content", {
    ...
    processContentElements: function (callback) {
        ...
        function innerProcessContentElements(callback) {
            var contentElements = Array.prototype.slice.call(xpath.select("//*[namespace-uri(.) != '' and namespace-uri(.) and namespace-uri(.) != 'http://www.w3.org/1999/xhtml']", document));
            ...
            if (contentElements.length) {
                flow()
                    .seqEach(contentElements, function (contentElement, cb) {
                        ...
                        var factory = require(Path.join(context.config.controllers[contentElement.namespaceURI], contentElement.localName));
                    ...
                    .exec(function (err) {
                        if (!err) {
                            innerProcessContentElements(callback);
                        } else {
                            callback(err);
                        }
                    });
    ...
    applyLayout: function (meta, callback) {
        var layout = this.meta.layout,
            self = this;


        // extends the meta data from the page
        _.each(this.meta, function (val, key) {
        ...
        if (layout) {
            ...
            flow()
                ...
                .seq("layout", function (cb) {
                    this.vars.layout.applyLayout(meta, cb);
                })
            ...
            flow().
                seq(function (cb) {
                    self.processContentElements(cb);
                })
                .exec(function (err) {
    ...
    createLayout: function (context, layout, callback) {
        ...
        flow()
            .seq("layout", function (cb) {
                fs.readFile(context.config.baseUri + "/layout/" + layout + ".html", "utf8", function (err, handle) {
